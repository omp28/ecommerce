      ...
      <nav className=" bg-white fixed w-full z-20 top-0 left-0 border-b border-gray-200">
        <div className=" max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
          ...
          <div className="flex md:order-2 gap-2 ">
            {/* account and cart for auth and non - admin user  */}
            ...
            {/* showing admin and client view according to user  */}
            {user?.role === "admin" ? (
              isAdminView ? (
                <button>Client</button>
              ) : (
                <button>AdminView</button>
              )
            ) : null}
            {isAuthUser ? <button>LogOut</button> : <button>LogIn</button>}
            {/* for mobile device 3 lines for opening and closing  */}
            <button
